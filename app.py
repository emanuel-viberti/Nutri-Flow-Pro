...
st.write(f"**Meta:** P: {int(g_prot)}g | G: {int(g_gras)}g | C: {int(g_carb)}g")
...
try:
    ...
    st.markdown("---")
    if st.button("🚀 Generar / Recalcular Menú"):
        if len(desayunos_f) < 2 or len(comidas_f) < 2:
            st.error("No hay suficientes platos con esos tags. Probá quitando algún filtro.")
        else:
            ...
            for i, dia in enumerate(dias):
                with cols[i]:
                    ...
                    # Intentamos 2000 combinaciones por día
                    for _ in range(2000):
                        ...
                        if dif < menor_dif:
                            menor_dif = dif
                            mejor_comb = (d, a, m, c, total)
                        
                        if dif <= margen: # Si está en el 5%, cortamos acá
                            break
                    
                    # Mostrar el mejor resultado encontrado
                    res_d, res_a, res_m, res_c, res_total = mejor_comb
                    st.info(f"**D:** {res_d['nombre']}")
                    st.success(f"**A:** {res_a['nombre']}")
                    st.info(f"**M:** {res_m['nombre']}")
                    st.success(f"**C:** {res_c['nombre']}")
                    
                    color_metrica = "normal" if menor_dif <= margen else "inverse"
                    st.metric("Total", f"{res_total} kcal", f"{res_total-kcal_final} dif", delta_color=color_metrica)
